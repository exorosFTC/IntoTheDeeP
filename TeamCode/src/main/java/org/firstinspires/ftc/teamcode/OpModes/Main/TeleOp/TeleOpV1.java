...
public class TeleOpV1 extends ExoMode implements Enums.IntakeEnums, Enums.OuttakeEnums {
    ...
    private final double intakeSensitivity = 0.7;
    ...
    protected void Loop() {
        ...
        switch (access) {
            case INTAKE: {
                ...
                if (robot.g2.wasJustPressed(GamepadKeys.Button.B))
                    robot.system.intake.lowerV4B();

                if (robot.g2.wasJustReleased(GamepadKeys.Button.B) && robot.system.intake.isV4bDown()) {
        ...
        robot.addTelemetry("Intake Position:", robot.system.intake.extension.getPosition());
