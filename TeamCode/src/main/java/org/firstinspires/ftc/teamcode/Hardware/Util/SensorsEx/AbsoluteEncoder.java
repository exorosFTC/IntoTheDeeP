...
import static org.firstinspires.ftc.teamcode.Hardware.Generals.Constants.SystemConstants.ENCODER_VALUE_REJECTION;
...
public class AbsoluteEncoder {
    ...
    public AbsoluteEncoder(HardwareMap hardwareMap, String encoder_name) { this(hardwareMap, encoder_name, range); }

    public AbsoluteEncoder(HardwareMap hardwareMap, String encoder_name, double actualRange) {
        encoder = hardwareMap.get(AnalogInput.class, encoder_name);
        this.range = actualRange;
    }

    public AbsoluteEncoder zero() {
    ...
    public double getPosition() {
        ...
        if(!ENCODER_VALUE_REJECTION || Math.abs(Angle.normDelta(last)) > 0.1 || Math.abs(Angle.normDelta(position)) < 1) last = position;
